...
**感受評分一律「越高越好」。**
`SHAVE_RATINGS`（`src/shared/domain.ts`）是唯一來源，表單的輸入列與日誌的顯示都從它長出來。
「刮傷」刻意用「舒適度」表達（5 分＝完全無感）：同一張表單裡若混著相反方向的量表，
填的時候極容易點反。要增減指標就改這份常數，再加上對應的欄位。

**收藏頁的分類篩選與搜尋做在前端。**
個人收藏是數十項的量級，一次抓齊換來的是分類按鈕上直接有數量、切換零延遲。
所以 `/api/items` 只保留 `status` 參數。
